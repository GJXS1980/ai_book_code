...
class Stm32ROS():
  def __init__(self):
    ...
    self.encoder_per_loop = int(rospy.get_param("~encoder_per_loop", 3188)) # 轮子转一圈编码输出多少脉冲
    ...
    while not rospy.is_shutdown():
    	...
    	# 开启消毒功能
      	...
      	# #	发布话题数据
      	# data = self.gyro_data_y
      	# self.gyro_pub.publish(data)
      	# print(data)
...
# 启动消毒功能
  def disinfect_switch(self, data):
    ...
    if self.disinfect_tag == 1:
      self.execute_array('x(1000)')

    # 启动２级消毒模式
    elif self.disinfect_tag == 2:
      self.execute_array('x(600)')

    # 启动３级消毒模式
    elif self.disinfect_tag == 3:
      self.execute_array('x(300)')

    # 启动４级消毒模式
    elif self.disinfect_tag == 4:
      self.execute_array('x(100)')
...
# 向下位机查询传感器数据(超声波、防碰撞、防跌落)
  ...
  def get_gyro_data(self, data):
    ...
    # if self.vA != 0 or self.vB != 0 or self.vC != 0 or self.vD != 0:
      ...
      # print(-self.gyro_data_y * 180 / pi)
        # # 测试是否运算对
      # print((gyro_values[0], gyro_values[1], gyro_values[2] - self.gyro_offset_y))
    # self.gyro_last = self.gyro_now
...
######################################################################################################
  ...
  def set_speed(self, motor_a, motor_d, motor_c, motor_b):
    motor_d = 0
    temp_translate = 1.0#1.476
    motor_a = float(motor_a) * self.meter_to_encoder * temp_translate / self.encoder_per_loop * 60.0
    motor_b = float(motor_b) * self.meter_to_encoder * temp_translate / self.encoder_per_loop * 60.0
    motor_c = float(motor_c) * self.meter_to_encoder * temp_translate / self.encoder_per_loop * 60.0
  ...
  def cmd_vel_callback(self, req):
    ...
    #2019.07.29增加内容
    # if abs(self.cmd_x) >= 0.23:
    #   self.cmd_x = (self.cmd_x / abs(self.cmd_x)) * 0.23
    #   rospy.loginfo("Current linear velocity exceeding the limit is limited to 0.240m/s.")
    # if abs(self.cmd_y) >= 0.23:
    #   self.cmd_y = (self.cmd_y / abs(self.cmd_y)) * 0.23
    #   rospy.loginfo("Current linear velocity exceeding the limit is limited to 0.240m/s.")
