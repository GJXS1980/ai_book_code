...
void execute_pick_up(const dobot::DobotGoalConstPtr& goal, Server* as)
{
    ...
    // 设置参数
    node.param("robot_init_x", robot_init_x, 200.0);    // 放置物体时机械臂的x坐标(笛卡尔坐标系)
    node.param("robot_init_y", robot_init_y, 0.0);    // 放置物体时机械臂的y坐标(笛卡尔坐标系)
    node.param("robot_init_z", robot_init_z, 60.0);    // 放置物体时机械臂的z坐标(笛卡尔坐标系)
...
// 创建PTP客户端
    ...
    dobot::SetPTPCmd srv;
        ...
        do {
            ...
            srv.request.x = robot_init_x;
            srv.request.y = robot_init_y;
            srv.request.z = robot_init_z;
            srv.request.r = 0;
            client.call(srv);
