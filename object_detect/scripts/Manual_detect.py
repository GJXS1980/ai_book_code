...
class Findposition:
    def __init__(self):
        ...
        self.get_color = Color['id']

        # 默认为紫色
        #self.get_color = rospy.set_param('~color_id', 4)

        # 音频文件的路径
        # self.voice = rospy.get_param("~success_file_path", "/params/identify_auido.mp3")
        ...
        rospy.Subscriber("control_success", Int64, self.getRobotcontrol_tag)
        rospy.Subscriber("input_rgb_image", Image, self.color_detect, queue_size=1)

        self.pub1 = rospy.Publisher('DetectDone', Int64, queue_size = 1)   
    ...
    #   获取机械臂识别物体位置初始化完成的tag
    def getRobotInit_tag(self,data):
        self.tag = data.data
        return self.tag

    #   获取机械臂完成运动的tag
    def getRobotcontrol_tag(self,data):
        self.tag2 = data.data
        return self.tag2
    ...
    def color_detect(self,data):
        ...
        if self.tag == 8 and color_judge:
            ...
            #   发布识别成功的信号
            if self.tag1 == 2 and self.center_ob != [0, 0] and self.tagf != 1 and self.side_judge and self.num == 0:
            	#playsound(self.voice)
            	#time.sleep(3)
            ...
            elif self.tag1 == 2 and self.center_ob != [0, 0] and self.tagf != 1 and self.side_judge and self.num >= 1 and self.tag2 == 3:
                #if self.num == 1:
                    #self.num += 1
                #else:
                    #playsound(self.voice)
