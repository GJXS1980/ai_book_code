...
int main(int argc, char** argv)
{
    ...
    ros::Publisher sk_data_pub = nh.advertise<castlex_msgs::skeleton>("skeleton", 10);
    Orbbec_Camera cam;
    ...
    while(ros::ok() && flag ) {
        ...
        if (!track.is_tracking() && !img.empty()) {
            imshow("image", img);
            // imshow("image", img);
            //waitKey(30);
            continue; 
