...
int main(int argc, char* argv[])
{
	...
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castle_xf_tts_topic", 3, xfCallBack);    //	订阅语义理解话题
	//ros::Subscriber sub2 = n.subscribe("/voice/castle_order_topic",orderCallBack);    //命令词识别失败时进行语音交互
	//pub = n.advertise<std_msgs::Int32>("/voice/castlex_xf_tts_topic", 1); 		//	发布语音合成话题
	//asr_pub = n.advertise<std_msgs::Int32>("/voice/castlex_xf_awake_topic", 1);  //	发布语音唤醒数据话题
	ros::NodeHandle nh("~");    //用于launch文件传递参数
	nh.param("appid", appid, string("appid = 5d7a01a4, work_dir = ."));    //从launch文件获取参数
